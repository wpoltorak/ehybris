...
public class FunctestRunSession implements ITestRunSession {
     ...
     */
    public FunctestRunSession(final String testRunName, final List<IJavaProject> projects) {
    ...
    public FunctestRunSession(final ILaunch launch, final List<IJavaProject> projects, final int port) {
    ...
    public boolean rerunTest(final String testId, final String className, final String testName, final String launchMode)
        ...
        if (fLaunch != null) {
            ...
            if (launchConfiguration != null) {
                ...
                final String configName = FunctestMessages.format(FunctestMessages.FunctestView_configName, name);
    ...
    public boolean rerunTests(final List<IJavaElement> elements, final String launchMode) throws CoreException {
        if (fLaunch != null) {
            // run the selected test using the previous launch configuration
            final ILaunchConfiguration launchConfiguration = fLaunch.getLaunchConfiguration();
            final FunctestLaunchConfigurationElements elems = new FunctestLaunchConfigurationElements(elements);

            if (launchConfiguration != null) {
                final String configName = FunctestMessages.format(FunctestMessages.FunctestView_configName, launchConfiguration.getName());

                final ILaunchConfigurationWorkingCopy tmp = launchConfiguration.copy(configName);
                // reset the container
                tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_PROJECT_NAMES, elems.getProjectNames());
                tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_METHOD_NAMES, elems.getMethodNames());
                tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAMES, elems.getMainTypeQualifiedNames());
                tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_CONTAINERS, elems.getContainerHandles());
                tmp.launch(launchMode, null);
                return true;
            }
        }
    ...
    private class TestSessionNotifier implements ITestRunListener2 {
        ...
        private TestSuiteElement getUnrootedSuite() {
            if (fUnrootedSuite == null) {
                fUnrootedSuite = (TestSuiteElement) createFunctestSuite(fTestRoot, "-2", FunctestMessages.TestRunSession_unrootedTests, 0); //$NON-NLS-1$
            }
