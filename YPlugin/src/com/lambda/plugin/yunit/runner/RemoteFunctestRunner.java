...
public class RemoteFunctestRunner implements MessageSender, IVisitsTestTrees {
	...
	protected void init(String[] args) {
		...
		if (projectClassMethodNames.keySet().isEmpty())
			throw new IllegalArgumentException(FunctestMessages
	...
	protected IFunctestLoader createRawTestLoader(int junitVersion) {
		// try {
		return new FunctestLoader(junitVersion);
		// } catch (Exception e) {
	...
	public void setLoader(IFunctestLoader newInstance) {
		fLoader = newInstance;
	}
	...
	protected boolean connect() {
		...
		runFailed(FunctestMessages.getFormattedString(
	...
	public IFunctestLoader getTestLoader() {
		return fLoader;
	}

	public Class loadClass(String className, RemoteFunctestRunner listener) {
		Class clazz = null;
		try {
			clazz = getTestClassLoader().loadClass(className);
		} catch (ClassNotFoundException e) {
			listener.runFailed(FunctestMessages.getFormattedString(
	...
	public static void main(String[] args) {
		try {
			...
			RemoteFunctestRunner testRunServer = new RemoteFunctestRunner();
				...
				while (true) {
					if ((message = fReader.readLine()) != null) {
						...
						if (message.startsWith(MessageIds.TEST_STOP)) {
							fStopped = true;
							RemoteFunctestRunner.this.stop();
							synchronized (RemoteFunctestRunner.this) {
								RemoteFunctestRunner.this.notifyAll();
							}
							break;
						}
