...
public class YUnitPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private class StackFilterContentProvider implements IStructuredContentProvider {
        ...
        public void saveFilters() {
            ...
            getPreferenceStore().setValue(PreferenceConstants.YUNIT_PREF_ACTIVE_STACK_FILTERS_LIST, pref);
            pref = PreferenceConstants.serializeList(inactive.toArray(new String[inactive.size()]));
            getPreferenceStore().setValue(PreferenceConstants.YUNIT_PREF_INACTIVE_STACK_FILTERS_LIST, pref);
        }
    ...
    private void editFilter() {
        ...
        /*
         * Workaround for https://bugs.eclipse.org/bugs/show_bug.cgi?id=218193 which won't get fixed (advice is to not
         * use border). We still use a border on platforms where it looks OK and nicer with a border.
         */
    ...
    /*
     * A valid step filter is simply one that is a valid Java identifier. and, as defined in the JDI spec, the regular
     * expressions used for step filtering must be limited to exact matches or patterns that begin with '*' or end with
     * '*'. Beyond this, a string cannot be validated as corresponding to an existing type or package (and this is
    ...
    protected List<String> createInactiveStackFiltersList() {
        String[] strings = YUnitPreferencePage.parseList(getPreferenceStore().getString(
                PreferenceConstants.YUNIT_PREF_INACTIVE_STACK_FILTERS_LIST));
    ...
    public static String[] getFilterPatterns() {
        ...
        return YUnitPreferencePage.parseList(store.getString(PreferenceConstants.YUNIT_PREF_ACTIVE_STACK_FILTERS_LIST));
    }

    public static boolean getFilterStack() {
        IPreferenceStore store = YPlugin.getDefault().getPreferenceStore();
        return store.getBoolean(PreferenceConstants.YUNIT_DO_FILTER_STACK);
    }

    public static void setFilterStack(boolean filter) {
        IPreferenceStore store = YPlugin.getDefault().getPreferenceStore();
        store.setValue(PreferenceConstants.YUNIT_DO_FILTER_STACK, filter);
    }
