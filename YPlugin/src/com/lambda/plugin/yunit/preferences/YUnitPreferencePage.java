...
public class YUnitPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private class StackFilterContentProvider implements IStructuredContentProvider {
		...
		public void saveFilters() {
			...
			getPreferenceStore().setValue(YUnitPreferencesConstants.PREF_ACTIVE_STACK_FILTERS_LIST, pref);
			pref = YUnitPreferencesConstants.serializeList(inactive.toArray(new String[inactive.size()]));
			getPreferenceStore().setValue(YUnitPreferencesConstants.PREF_INACTIVE_STACK_FILTERS_LIST, pref);
		}
	...
	protected List<String> createInactiveStackFiltersList() {
		String[] strings = YUnitPreferencePage.parseList(getPreferenceStore().getString(
				YUnitPreferencesConstants.PREF_INACTIVE_STACK_FILTERS_LIST));
	...
	public static String[] getFilterPatterns() {
		...
		return YUnitPreferencePage.parseList(store.getString(YUnitPreferencesConstants.PREF_ACTIVE_STACK_FILTERS_LIST));
	}

	public static boolean getFilterStack() {
		IPreferenceStore store = YPlugin.getDefault().getPreferenceStore();
		return store.getBoolean(YUnitPreferencesConstants.DO_FILTER_STACK);
	}

	public static void setFilterStack(boolean filter) {
		IPreferenceStore store = YPlugin.getDefault().getPreferenceStore();
		store.setValue(YUnitPreferencesConstants.DO_FILTER_STACK, filter);
	}
