...
public class FunctestModel {
    ...
    private final class JUnitLaunchListener implements ILaunchListener {
        ...
        public void launchChanged(final ILaunch launch) {
            ...
            final String portStr = launch.getAttribute(FunctestLaunchConfigurationConstants.ATTR_PORT);
            if (portStr == null) {
                return;
            }
            try {
        ...
        private void connectTestRunner(final ILaunch launch, final Map<String, IJavaProject> javaProjects, final int port) {
            ...
            // asyncExec())
            final int maxCount = YPlugin.getDefault().getPreferenceStore().getInt(FunctestPreferencesConstants.MAX_TEST_RUNS);
            int toDelete = fFunctestRunSessions.size() - maxCount;
            while (toDelete > 0) {
                toDelete--;
                final FunctestRunSession session = fFunctestRunSessions.removeLast();
                notifyFunctestRunSessionRemoved(session);
            }

            final FunctestRunSession testRunSession = new FunctestRunSession(launch, new ArrayList<IJavaProject>(javaProjects.values()),
                    port);
            addFunctestRunSession(testRunSession);
        }

        private FunctestView showTestRunnerViewPartInActivePage(final FunctestView testRunner) {
    ...
    public void addFunctestRunSessionListener(final IFunctestRunSessionListener listener) {
        fFunctestRunSessionListeners.add(listener);
    }

    public void removeFunctestRunSessionListener(final IFunctestRunSessionListener listener) {
        fFunctestRunSessionListeners.remove(listener);
    }

    /**
     * @return a list of active {@link FunctestRunSession}s. The list is a copy of the internal data structure and modifications do not
    ...
    /**
     * Adds the given {@link FunctestRunSession} and notifies all registered {@link IFunctestRunSessionListener}s.
     * <p>
     ...
     */
    public void addFunctestRunSession(final FunctestRunSession testRunSession) {
     ...
     */
    public static FunctestRunSession importFunctestRunSession(final File file) throws CoreException {
        try {
            final SAXParserFactory parserFactory = SAXParserFactory.newInstance();
            // parserFactory.setValidating(true); // TODO: add DTD and debug flag
            final SAXParser parser = parserFactory.newSAXParser();
            final FunctestRunHandler handler = new FunctestRunHandler();
            parser.parse(file, handler);
            final FunctestRunSession session = handler.getTestRunSession();
    ...
    public static void importIntoFunctestRunSession(final File swapFile, final FunctestRunSession testRunSession) throws CoreException {
        try {
            final SAXParserFactory parserFactory = SAXParserFactory.newInstance();
            // parserFactory.setValidating(true); // TODO: add DTD and debug flag
            final SAXParser parser = parserFactory.newSAXParser();
            final FunctestRunHandler handler = new FunctestRunHandler(testRunSession);
     ...
     */
    public static void exportFunctestRunSession(final FunctestRunSession testRunSession, final File file) throws CoreException {
    ...
    public static void exportFunctestRunSession(final FunctestRunSession testRunSession, final OutputStream out)
            throws TransformerFactoryConfigurationError, TransformerException {

        final Transformer transformer = TransformerFactory.newInstance().newTransformer();
        final InputSource inputSource = new InputSource();
        final SAXSource source = new SAXSource(new FunctestRunSessionSerializer(testRunSession), inputSource);
    ...
    private static void throwExportError(final File file, final Exception e) throws CoreException {
        throw new CoreException(new org.eclipse.core.runtime.Status(IStatus.ERROR, YPlugin.PLUGIN_ID, FunctestMessages.format(
                FunctestMessages.FunctestModel_could_not_write, BasicElementLabels.getPathLabel(file)), e));
    }

    private static void throwImportError(final File file, final Exception e) throws CoreException {
        throw new CoreException(new org.eclipse.core.runtime.Status(IStatus.ERROR, YPlugin.PLUGIN_ID, FunctestMessages.format(
                FunctestMessages.FunctestModel_could_not_read, BasicElementLabels.getPathLabel(file)), e));
    }

    /**
     * Removes the given {@link FunctestRunSession} and notifies all registered {@link IFunctestRunSessionListener}s.
     * <p>
     * <b>To be called in the UI thread only!</b>
     * </p>
     * 
     ...
     */
    public void removeFunctestRunSession(final FunctestRunSession testRunSession) {
    ...
    private void notifyFunctestRunSessionRemoved(final FunctestRunSession testRunSession) {
    ...
    private void notifyFunctestRunSessionAdded(final FunctestRunSession testRunSession) {
        final Object[] listeners = fFunctestRunSessionListeners.getListeners();
        for (int i = 0; i < listeners.length; ++i) {
