...
public class YUnitPropertyPage extends PropertyPage {
	...
	private class TestFilterContentProvider implements IStructuredContentProvider {
		...
		public void setDefaults() {
			...
			List<String> active = YUnitPropertyConstants.createDefaultTestFiltersList();
		...
		public void saveFilters() {
			...
			try {
				String pref = YUnitPropertyConstants.serializeList(active.toArray(new String[active.size()]));
				((IResource)getElement().getAdapter(IResource.class)).setPersistentProperty(new QualifiedName("", YUnitPropertyConstants.PREF_ACTIVE_TEST_FILTERS_LIST), pref);
				pref = YUnitPropertyConstants.serializeList(inactive.toArray(new String[inactive.size()]));
				((IResource)getElement().getAdapter(IResource.class)).setPersistentProperty(new QualifiedName("", YUnitPropertyConstants.PREF_INACTIVE_TEST_FILTERS_LIST), pref);
	...
	protected List<String> createInactiveTestFiltersList() {
		...
		try {
			String persistentProperty = ((IResource) getElement().getAdapter(IResource.class)).getPersistentProperty(new QualifiedName("", YUnitPropertyConstants.PREF_INACTIVE_TEST_FILTERS_LIST));
			String[] strings = YUnitPropertyPage.parseList(persistentProperty == null ? "" : persistentProperty);
	...
	public String[] getFilterPatterns() throws CoreException {
		String persistentProperty = ((IResource) getElement().getAdapter(IResource.class)).getPersistentProperty(new QualifiedName("", YUnitPropertyConstants.PREF_ACTIVE_TEST_FILTERS_LIST));
		return YUnitPropertyPage.parseList(persistentProperty == null ? "" : persistentProperty);
	}
