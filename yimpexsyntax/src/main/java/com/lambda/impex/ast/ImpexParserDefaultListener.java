...
public class ImpexParserDefaultListener extends ImpexParserBaseListener {
	...
	private static final Set<String> javaKeywords = new HashSet<String>(Arrays.asList("abstract", "assert", "boolean",
	...
	public void enterHeaderTypeName(final HeaderTypeNameContext ctx) {
		...
		if (ctx.Type() != null) {
			context.addType(ctx.Type().getSymbol().getStartIndex(), typeDescription.getTarget());
		}
	...
	public void enterAttribute(final AttributeContext ctx) {
		...
		// final ImpexParserDefaultListener listener = new
		// ImpexParserDefaultListener(model);
...
//		currentColumnDescription = listener.getColumnDescription();
	...
	public void enterSimpleAttributeName(final SimpleAttributeNameContext ctx) {
		...
		if (owner == null) {
			// already reported InvalidAttribute error in
			// enterSimpleAttributeName(method)
			return;
		}
    ...
    public void enterAttributeModifierAssignment(final AttributeModifierAssignmentContext ctx) {
        ...
        if (unknownModifier != null) {
            context.addProblem(problem(unknownModifier, Type.UnknownAttributeModifier));
            return;
        }
        ...
        Token start = attributeModifier.getStart();
		switch (start.getType()) {
            ...
            case ImpexLexer.ClassAttributeModifier:
                ...
                String attributename = start.getText().trim().toLowerCase();
                switch (attributename){
                case "celldecorator":
                	Object decoratorType = typeFinder.findDecoratorType(text);
                	if (decoratorType != null) {
                		context.addType(modifierval.getStart().getStartIndex(), decoratorType);
                	}
                	break;
                case "translator":
                	Object translatorType = typeFinder.findTranslatorType(text);
                	if (translatorType != null) {
                		context.addType(modifierval.getStart().getStartIndex(), translatorType);
                	}
                	break;
                }
                break;
	...
	public void enterDocumentIdField(final DocumentIdFieldContext ctx) {
		...
		context.addDocumentIDDefinitionQualifier(new DocumentIDDescription(column.getDocumentID(), text),
				typeDescription, ctx.DocumentIdField().getSymbol());
	}
	...
	public void enterDocumentIdRefField(final DocumentIdRefFieldContext ctx) {
		...
		context.addDocumentIDReferenceQualifier(new DocumentIDDescription(column.getDocumentID(), text), type,
				ctx.DocumentIdRefField().getSymbol());
	}
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * final ImpexProblem problem = new ImpexProblem( The default implementation
	 * does nothing.
	 */
	@Override
