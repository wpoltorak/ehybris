...
public class KizoomActionsMenu implements IObjectActionDelegate, IActionDelegate, IMenuCreator {
			...
			if (extensionPoint != null) {
				for (IExtension extension : extensionPoint.getExtensions()) {
					...
					for (IConfigurationElement element : elements) {
						try {
							AbstractActionMenuCreator creator = (AbstractActionMenuCreator) element
									.createExecutableExtension("class");
							creators.add(creator);
						} catch (CoreException ex) {
							Logger.getLogger(KizoomPlugin.LOGGER_NAME).log(Level.WARNING, ex.getMessage());
						}
					}
				}
			}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if (selection instanceof IStructuredSelection) {
			...
			for (AbstractActionMenuCreator creator : getCreators()) {
	...
	private class ProjectMenuListener extends MenuAdapter {
		...
		public void menuShown(MenuEvent e) {
			if (fillMenu) {
				...
				IMenuManager mgr = new MenuManager("#kizoom");
