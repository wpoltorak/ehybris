...
public class YUnitLoader implements IYUnitLoader {
	// JUNIT3
	...
	public YUnitLoader(int junitVersion) {
...
//							test = setupTest(testClass, createTest(methods.get(0), testClass));
	...
	private Test setupTest(Class<?> reloadedTestClass, Test reloadedTest) {
		...
		if (setup.getReturnType() != Test.class)
			return warning(YUnitMessages.getString("RemoteFunctestRunner_error_notestreturn")); //$NON-NLS-1$
		if (!Modifier.isPublic(setup.getModifiers()))
			return warning(YUnitMessages.getString("RemoteFunctestRunner_error_shouldbepublic")); //$NON-NLS-1$
		if (!Modifier.isStatic(setup.getModifiers()))
			return warning(YUnitMessages.getString("RemoteFunctestRunner_error_shouldbestatic")); //$NON-NLS-1$
		try {
			Test test = (Test) setup.invoke(null, new Object[] { reloadedTest });
			if (test == null)
				return warning(YUnitMessages.getString("RemoteFunctestRunner_error_nullreturn")); //$NON-NLS-1$
			return test;
		} catch (IllegalArgumentException e) {
			return warning(YUnitMessages.getFormattedString("RemoteFunctestRunner_error_couldnotinvoke", e)); //$NON-NLS-1$
		} catch (IllegalAccessException e) {
			return warning(YUnitMessages.getFormattedString("RemoteTestRunner_error_couldnotinvoke", e)); //$NON-NLS-1$
		} catch (InvocationTargetException e) {
			return warning(YUnitMessages.getFormattedString(
					"RemoteTestRunner_error_invocationexception", e.getTargetException())); //$NON-NLS-1$
		}
	}

	public Test getJunit3Test(Class<?> testClass, RemoteYUnitRunner failureListener) {
		Method suiteMethod = null;
		try {
			suiteMethod = testClass.getMethod(YUnitLoader.SUITE_METHODNAME, new Class[0]);
		} catch (Exception e) {
			// try to extract a test suite automatically
			return new TestSuite(testClass);
		}
		if (!Modifier.isStatic(suiteMethod.getModifiers())) {
			return warning(YUnitMessages.getString("RemoteTestRunner_error_suite_notstatic"));//$NON-NLS-1$
		}
		try {
			...
			return warning(YUnitMessages.getString("RemoteTestRunner_error_suite_nullreturn")); //$NON-NLS-1$
		} catch (InvocationTargetException e) {
			String message = YUnitMessages.getFormattedString(
					"RemoteTestRunner_error_invoke", e.getTargetException().toString()); //$NON-NLS-1$
			failureListener.runFailed(message, e);
			return new TestSuite(testClass);
		} catch (IllegalAccessException e) {
			String message = YUnitMessages.getFormattedString("RemoteTestRunner_error_invoke", e.toString()); //$NON-NLS-1$
			failureListener.runFailed(message, e);
			return new TestSuite(testClass);
		}
	}

	public Test getJunit3Test(Class<?> testClass, List<String> testMethods, RemoteYUnitRunner failureListener) {
