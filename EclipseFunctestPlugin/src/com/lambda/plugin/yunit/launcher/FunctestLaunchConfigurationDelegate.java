...
import com.lambda.plugin.yunit.properties.FunctestPropertyConstants;
...
public class FunctestLaunchConfigurationDelegate extends AbstractJavaLaunchConfigurationDelegate {
    ...
    protected IMember[] filterTests(final IMember[] members) throws CoreException {
        ...
        for (final IMember member : members) {
            final String persistentProperty = member.getJavaProject().getProject()
                    .getPersistentProperty(new QualifiedName("", FunctestPropertyConstants.PREF_ACTIVE_TEST_FILTERS_LIST));
            final String[] patterns = parseList(persistentProperty == null ? "" : persistentProperty);
            final String memberStr = FunctestLaunchConfigurationUtils.toString(member);
    ...
    protected IMember[] evaluateTests(final ILaunchConfiguration configuration, final IProgressMonitor monitor) throws CoreException {
        final Map<String, IJavaProject> javaProjects = FunctestLaunchConfigurationUtils.getJavaProjects(configuration);
        ...
        final String testMethodNames = configuration.getAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_METHOD_NAMES, ""); //$NON-NLS-1$
        if (testMethodNames.length() > 0) {
            final String[] methodNames = testMethodNames.split(",");
            for (final String methodName : methodNames) {
                final String[] projectTypeAndMethod = FunctestLaunchConfigurationUtils.split(methodName);
    ...
    protected void collectExecutionArguments(final ILaunchConfiguration configuration, final List<String> vmArguments,
        ...
        final String testFailureNames = configuration.getAttribute(FunctestLaunchConfigurationConstants.ATTR_FAILURES_NAMES, ""); //$NON-NLS-1$
        final String testServer = configuration.getAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_SERVER, (String) null);
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < testElements.length; i++) {
                    final String entry = FunctestLaunchConfigurationUtils.toString(testElements[i], '$');
                    if (entry != null) {
                        bw.write(entry);
                        bw.newLine();
                    } else {
