...
import com.kizoom.plugin.functest.launcher.FunctestLaunchConfigurationElements;

public class FunctestRunSession implements ITestRunSession {
	 ...
	 */
	public FunctestRunSession(String testRunName, List<IJavaProject> projects) {
	...
	public FunctestRunSession(ILaunch launch, List<IJavaProject> projects, int port) {
		...
		if (launchConfiguration != null) {
			...
			fTestRunnerKind = FunctestLaunchConfigurationConstants.getTestRunnerKind(launchConfiguration);
		} else {
		...
		try {
			fTestServer  = launchConfiguration.getAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_SERVER, "unknown");
		...
		fSessionListeners = new ListenerList();
		addTestSessionListener(new FunctestRunListenerAdapter(this));
	}
	...
	public void swapOut() {
		...
		try {
			...
			FunctestModel.exportFunctestRunSession(this, swapFile);
	...
	private File getSwapFile() throws IllegalStateException {
		File historyDir = FunctestPlugin.getHistoryDirectory();
	...
	public void swapIn() {
		...
		try {
			FunctestModel.importIntoFunctestRunSession(getSwapFile(), this);
		} catch (IllegalStateException e) {
			KizoomPlugin.logError(e);
			fTestRoot = new TestRoot(this);
			fTestResult = null;
	...
	public boolean rerunTest(String testId, String className, String testName, String launchMode) throws CoreException {
			...
			if (launchConfiguration != null) {
				...
				String configName = FunctestMessages.format(FunctestMessages.FunctestView_configName, name);
				ILaunchConfigurationWorkingCopy tmp = launchConfiguration.copy(configName);
				// reset the container
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAMES, ""); //$NON-NLS-1$
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_CONTAINERS, ""); //$NON-NLS-1$
				if (testName != null) {
					tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_METHOD_NAMES, testName);
				}
	...
	public boolean rerunTests(List<IJavaElement> elements, String launchMode) throws CoreException {
		if (fLaunch != null) {
			// run the selected test using the previous launch configuration
			ILaunchConfiguration launchConfiguration = fLaunch.getLaunchConfiguration();
			FunctestLaunchConfigurationElements elems = new FunctestLaunchConfigurationElements(elements);
			
			if (launchConfiguration != null) {
				String configName = FunctestMessages.format(FunctestMessages.FunctestView_configName, launchConfiguration.getName());
				
				ILaunchConfigurationWorkingCopy tmp = launchConfiguration.copy(configName);
				// reset the container
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_PROJECT_NAMES, elems.getProjectNames()); //$NON-NLS-1$
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_METHOD_NAMES, elems.getMethodNames()); //$NON-NLS-1$
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAMES, elems.getMainTypeQualifiedNames()); //$NON-NLS-1$
				tmp.setAttribute(FunctestLaunchConfigurationConstants.ATTR_TEST_CONTAINERS, elems.getContainerHandles()); //$NON-NLS-1$
				tmp.launch(launchMode, null);
				return true;
			}
		}
	...
	public TestElement createFunctestSuite(TestSuiteElement parent, String id, String testName, 
			int testCount) {
		TestSuiteElement testSuiteElement = new TestSuiteElement(parent, id, testName, testCount);
		if (testCount > 0)
			fIncompleteTestSuites.add(new IncompleteTestSuite(testSuiteElement, testCount));
	...
	private class TestSessionNotifier implements ITestRunListener2 {
		...
		private TestSuiteElement getUnrootedSuite() {
			if (fUnrootedSuite == null) {
				fUnrootedSuite = (TestSuiteElement) createFunctestSuite(fTestRoot,
						"-2", FunctestMessages.TestRunSession_unrootedTests, 0); //$NON-NLS-1$
			}
		 ...
		 * 
		 * @see
		 * org.eclipse.jdt.internal.junit.model.ITestRunListener2#testFailed(int,
		 ...
		 * .lang.String, java.lang.String, java.lang.String, int, java.lang.String,
		...
		private void logUnexpectedTest(String testId, TestElement testElement) {
			KizoomPlugin.logError(new Exception("Unexpected TestElement type for testId '" + testId + "': " + testElement)); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
