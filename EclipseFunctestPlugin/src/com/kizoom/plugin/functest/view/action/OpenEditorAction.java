...
public abstract class OpenEditorAction extends Action {
	...
	protected OpenEditorAction(FunctestView testRunner, String testClassName) {
		this(testRunner, testClassName, true);
	}

	public OpenEditorAction(FunctestView testRunner, String className, boolean activate) {
	...
	public void run() {
		...
		try {
			IJavaElement element = findElement(getLaunchedProjects(), fClassName);
			if (element == null) {
	...
	protected final IType findType(final List<IJavaProject> projects, String className) {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					try {
						if (projects == null) {
							...
							new SearchEngine().searchAllTypeNames(lastDot >= 0 ? dottedName.substring(0, lastDot).toCharArray()
									: null, SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE, (lastDot >= 0 ? dottedName
									.substring(lastDot + 1) : dottedName).toCharArray(), SearchPattern.R_EXACT_MATCH
									| SearchPattern.R_CASE_SENSITIVE, IJavaSearchConstants.TYPE, SearchEngine.createWorkspaceScope(),
