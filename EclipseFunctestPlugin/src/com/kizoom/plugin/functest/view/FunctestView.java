...
import com.kizoom.plugin.functest.IFunctestRunSessionListener;
import com.kizoom.plugin.functest.preferences.FunctestPreferencesConstants;
import com.kizoom.plugin.functest.view.action.ActivateOnErrorAction;
import com.kizoom.plugin.functest.view.action.FunctestCopyAction;
import com.kizoom.plugin.functest.view.action.RerunLastAction;
import com.kizoom.plugin.functest.view.action.RerunLastFailedOnlyAction;
import com.kizoom.plugin.functest.view.action.ScrollLockAction;
import com.kizoom.plugin.functest.view.action.ShowNextFailureAction;
import com.kizoom.plugin.functest.view.action.ShowPreviousFailureAction;
import com.kizoom.plugin.functest.view.action.ShowTimeAction;
import com.kizoom.plugin.functest.view.action.StopAction;
import com.kizoom.plugin.functest.view.action.ToggleOrientationAction;

@SuppressWarnings("restriction")
public class FunctestView extends ViewPart {
	...
	private FunctestCopyAction fCopyAction;
	...
	private class TestRunSessionListener implements IFunctestRunSessionListener {
		public void sessionAdded(FunctestRunSession testRunSession) {
			if (getSite().getWorkbenchWindow() == KizoomPlugin.getActiveWorkbenchWindow()) {
				FunctestRunSession deactivatedSession = setActiveTestRunSession(testRunSession);
				...
				if (testRunSession.getLaunch() != null) {
					msg = FunctestMessages.format(FunctestMessages.FunctestView_Launching, new Object[] { testRunLabel });
				} else {
					msg = testRunLabel;
				}
		...
		public void sessionRemoved(FunctestRunSession testRunSession) {
			if (testRunSession.equals(fTestRunSession)) {
				...
				FunctestRunSession deactivatedSession;
	...
	private class TestSessionListener implements ITestSessionListener {
		...
		public void sessionEnded(long elapsedTime) {
			...
			String msg = FunctestMessages.format(FunctestMessages.FunctestView_message_finish, keys);
		...
		public void sessionStopped(final long elapsedTime) {
			...
			registerInfoMessage(FunctestMessages.FunctestView_message_stopped);
			handleStopped();
		}
		...
		public void sessionTerminated() {
			...
			registerInfoMessage(FunctestMessages.FunctestView_message_terminated);
			handleStopped();
		}
		...
		public void testStarted(TestCaseElement testCaseElement) {
			...
			String status = FunctestMessages.format(FunctestMessages.FunctestView_message_started, new String[] { className,
					method });
	...
	private class FunctestIsRunningJob extends Job {
		public FunctestIsRunningJob(String name) {
			super(name);
			setSystem(true);
		}
		...
		public boolean belongsTo(Object family) {
			return family == FunctestView.FAMILY_JUNIT_RUN;
		}
	}
	...
	private class FailuresOnlyFilterAction extends Action {
		public FailuresOnlyFilterAction() {
			super(FunctestMessages.FunctestView_show_failures_only, AS_CHECK_BOX);
			setToolTipText(FunctestMessages.FunctestView_show_failures_only);
	...
	private class ShowTestHierarchyAction extends Action {
		...
		public ShowTestHierarchyAction() {
			super(FunctestMessages.FunctestView_hierarchical_layout, IAction.AS_CHECK_BOX);
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		if (progressService != null)
			progressService.showBusyForFamily(FunctestView.FAMILY_JUNIT_RUN);
	}
	...
	public void stopTest() {
		if (fTestRunSession != null) {
			if (fTestRunSession.isRunning()) {
				setContentDescription(FunctestMessages.FunctestView_message_stopping);
			}
	...
	private void startUpdateJobs() {
		...
		fFunctestIsRunningJob = new FunctestIsRunningJob(FunctestMessages.FunctestView_wrapperJobName);
		...
		fUpdateJob = new UpdateUIJob(FunctestMessages.FunctestView_jobName);
	...
	private void updateViewIcon() {
		if (fTestRunSession == null || fTestRunSession.isStopped() || fTestRunSession.isRunning()
			...
			fViewImage = FunctestViewImages.fTestRunFailIcon;
		else
			fViewImage = FunctestViewImages.fTestRunOKIcon;
		firePropertyChange(IWorkbenchPart.PROP_TITLE);
	}
	 ...
	 *         deactivated
	 */
	FunctestRunSession setActiveTestRunSession(FunctestRunSession testRunSession) {
		/*
		...
		FunctestRunSession deactivatedSession = fTestRunSession;
	...
	private void setTitleToolTip() {
		...
		if (testKindDisplayStr != null)
			setTitleToolTip(MessageFormat.format(FunctestMessages.FunctestView_titleToolTip, new String[] { testRunLabel,
					testKindDisplayStr }));
		else
	...
	public synchronized void dispose() {
		...
		if (fTestRunSessionListener != null)
			FunctestPlugin.getModel().removeFunctestRunSessionListener(fTestRunSessionListener);
	...
	public void showTestResultsView() {
		...
		FunctestView testRunner = null;
		if (page != null) {
			try { // show the result view
				testRunner = (FunctestView) page.findView(FunctestView.NAME);
				if (testRunner == null) {
					IWorkbenchPart activePart = page.getActivePart();
					testRunner = (FunctestView) page.showView(FunctestView.NAME);
					// restore focus
	...
	private SashForm createSashForm(Composite parent) {
		...
		label.setText(FunctestMessages.FunctestView_label_failure);
		label.setImage(FunctestViewImages.fStackViewIcon);
	...
	public void createPartControl(Composite parent) {
		...
		fCopyAction = new FunctestCopyAction(fFailureTrace, fClipboard);
		...
		FunctestPlugin.getModel().addFunctestRunSessionListener(fTestRunSessionListener);
	}
	...
	public void computeOrientation(int fOrientation) {
	...
	private void configureToolBar() {
		...
		MenuManager layoutSubMenu = new MenuManager(FunctestMessages.FunctestView_layout_menu);
	...
	void codeHasChanged() {
		...
		if (fViewImage == FunctestViewImages.fTestRunOKIcon)
			fViewImage = FunctestViewImages.fTestRunOKDirtyIcon;
		else if (fViewImage == FunctestViewImages.fTestRunFailIcon)
			fViewImage = FunctestViewImages.fTestRunFailDirtyIcon;
	...
	public void rerunTest(String testId, String className, String testName, String launchMode) {
		...
		try {
			...
			if (!couldLaunch) {
				MessageDialog.openInformation(getSite().getShell(), FunctestMessages.FunctestView_cannotrerun_title,
						FunctestMessages.FunctestView_cannotrerurn_message);
			}
		} catch (CoreException e) {
			ErrorDialog.openError(getSite().getShell(), FunctestMessages.FunctestView_error_cannotrerun, e.getMessage(), e
					.getStatus());
		}
	}
	
	public void rerunTests(List<IJavaElement> elements, String launchMode) {
		DebugUITools.saveAndBuildBeforeLaunch();
		try {
			boolean couldLaunch = fTestRunSession.rerunTests(elements, launchMode);
			if (!couldLaunch) {
				MessageDialog.openInformation(getSite().getShell(), FunctestMessages.FunctestView_cannotrerun_title,
						FunctestMessages.FunctestView_cannotrerurn_message);
			}
	...
	private static boolean getShowOnErrorOnly() {
		...
		return store.getBoolean(FunctestPreferencesConstants.SHOW_ON_ERROR_ONLY);
	}
	...
	public FunctestRunSession getTestRunSession() {
